...
import { normalizeBrazilianWhatsAppNumber } from '@/lib/outreach'
import type {
  CrmAccount,
...
const WHATSAPP_SYNC_JOB_TYPES = ['whatsapp_poll', 'whatsapp_webhook'] as const
...
function normalizeWhatsAppIdentifier(value: string | null | undefined) {
  ...
  return normalizeBrazilianWhatsAppNumber(digits) ?? (digits.length >= 8 ? digits : null)
}

function buildMatchToken(normalizedNumber: string) {
  if (normalizedNumber.length <= 8) {
    return normalizedNumber
  }

  return normalizedNumber.slice(-8)
}

function numbersLikelyMatch(left: string | null, right: string | null) {
  if (!left || !right) {
    return false
  }
...
function buildExcerpt(input: Pick<CrmWhatsAppSyncMessageInput, 'excerpt' | 'body'>) {
  return sanitizeText(input.excerpt, 240) ?? sanitizeText(input.body, 240)
}

function getOccurredAt(message: CrmWhatsAppSyncMessageInput) {
...
function isWhatsAppSyncJobType(value: string): value is WhatsAppSyncJobType {
  return WHATSAPP_SYNC_JOB_TYPES.includes(value as WhatsAppSyncJobType)
}
...
function isSyncableWhatsAppDirection(value: unknown): value is CrmMessageDirection {
  return value === 'outbound' || value === 'inbound'
}

export function parseCrmWhatsAppSyncPayload(
  payload: unknown
): { data: CrmWhatsAppSyncPayload } | { error: string; details?: string } {
  ...
  if (unknownFields.length > 0) {
    return {
      error: 'Unknown WhatsApp sync fields in payload.',
  ...
  if (
    ...
    !isWhatsAppSyncJobType(input.jobType)
  ) {
    return {
      error: 'Invalid jobType value.',
      details: 'Use one of: whatsapp_poll, whatsapp_webhook.',
    }
  }
  ...
  if (input.messages.length > MAX_SYNC_MESSAGES) {
    return {
      error: 'Too many WhatsApp sync messages.',
      details: `Maximum supported batch size is ${MAX_SYNC_MESSAGES}.`,
    }
  }

  const messages: CrmWhatsAppSyncMessageInput[] = []
  ...
  for (const [index, rawMessage] of input.messages.entries()) {
    ...
    if (!isSyncableWhatsAppDirection(message.direction)) {
      return {
    ...
    const senderIdentifier = normalizeWhatsAppIdentifier(
      typeof message.senderIdentifier === 'string'
        ? message.senderIdentifier
        : null
    )
    const recipientIdentifier = normalizeWhatsAppIdentifier(
      typeof message.recipientIdentifier === 'string'
        ? message.recipientIdentifier
        : null
    )

    if (!accountId && !senderIdentifier && !recipientIdentifier && !conversationId) {
      return {
        error: 'Insufficient WhatsApp matching data.',
        details: `Message at index ${index} must include accountId, senderIdentifier, recipientIdentifier, or conversationId.`,
      }
    }
    ...
    messages.push({
      ...
      receivedAt: normalizeTimestamp(message.receivedAt),
...
type PersistedWhatsAppOutreachEvent = {
...
async function loadWhatsAppConnection(
  ...
  if (!connection.data) {
    throw new Error('CRM WhatsApp connection not found.')
  }

  const data = connection.data as CrmChannelConnection

  if (data.channel !== 'whatsapp') {
    throw new Error('The provided CRM connection is not a WhatsApp channel.')
  }

  if (!isCrmConnectionConnected(data.connection_status)) {
    throw new Error('The CRM WhatsApp connection is not active.')
  }
...
async function createSyncJob(
  supabase: SupabaseClient,
  payload: CrmWhatsAppSyncPayload
) {
  ...
  if (createdJob.error || !createdJob.data) {
    throw new Error(
      createdJob.error?.message ?? 'Failed to create CRM WhatsApp sync job.'
    )
  }
...
async function touchConnectionAfterSync(
  ...
  const update = await supabase
    ...
    .update({
      last_synced_at: now,
      last_webhook_at: jobType === 'whatsapp_webhook' ? now : undefined,
      last_error:
        result.errors.length > 0
...
async function resolveAccountByWhatsApp(
  supabase: SupabaseClient,
  normalizedPhone: string
) {
  const token = buildMatchToken(normalizedPhone)

  const contacts = await supabase
    .from('crm_contacts')
    .select('id, account_id, whatsapp, phone')
    .or(`whatsapp.ilike.%${token}%,phone.ilike.%${token}%`)
    .limit(25)
  ...
  const matchedContact = (contacts.data ?? []).find((contact) =>
    numbersLikelyMatch(normalizeWhatsAppIdentifier(contact.whatsapp), normalizedPhone) ||
    numbersLikelyMatch(normalizeWhatsAppIdentifier(contact.phone), normalizedPhone)
  )

  if (matchedContact) {
    return {
      accountId: matchedContact.account_id as string,
      contactId: matchedContact.id as string,
    }
  }
  ...
  const matchedAccount = (accounts.data ?? []).find((account) =>
    numbersLikelyMatch(normalizeWhatsAppIdentifier(account.phone), normalizedPhone)
  )

  if (matchedAccount) {
    return {
      accountId: matchedAccount.id as string,
      contactId: null,
    }
  }
...
async function resolveAccountContextForMessage(
  supabase: SupabaseClient,
  message: CrmWhatsAppSyncMessageInput
  ...
  if (!accountId && message.conversationId) {
    const threadedMessage = await supabase
      .from('crm_messages')
      .select('id, account_id, contact_id, deal_id, outreach_event_id')
  ...
  if (!accountId) {
    ...
    if (phoneCandidate) {
      const matched = await resolveAccountByWhatsApp(supabase, phoneCandidate)
...
async function upsertCrmMessage(
...
): Promise<PersistedWhatsAppMessage> {
      ...
      if (updatedMessage.error || !updatedMessage.data) {
        throw new Error(
          updatedMessage.error?.message ??
            'Failed to update CRM WhatsApp message.'
        )
      }
  ...
  if (insertedMessage.error || !insertedMessage.data) {
    throw new Error(
      insertedMessage.error?.message ?? 'Failed to insert CRM WhatsApp message.'
    )
  }
...
async function upsertCrmOutreachEvent(
...
): Promise<PersistedWhatsAppOutreachEvent> {
  if (payload.external_message_id) {
    ...
    if (existingEvent.data) {
      ...
      if (updatedEvent.error || !updatedEvent.data) {
        throw new Error(
          updatedEvent.error?.message ??
            'Failed to update CRM WhatsApp outreach event.'
        )
      }
  ...
  if (insertedEvent.error || !insertedEvent.data) {
    throw new Error(
      insertedEvent.error?.message ??
        'Failed to insert CRM WhatsApp outreach event.'
    )
  }
...
async function updateAccountAfterOutboundWhatsApp(
  supabase: SupabaseClient,
  account: CrmAccount,
  message: CrmWhatsAppSyncMessageInput,
  excerpt: string | null
...
async function updateAccountAfterInboundWhatsApp(
  supabase: SupabaseClient,
  account: CrmAccount,
  message: CrmWhatsAppSyncMessageInput,
  excerpt: string | null,
...
async function processWhatsAppSyncMessage(
  supabase: SupabaseClient,
  message: CrmWhatsAppSyncMessageInput
) {
  ...
  if (message.direction === 'outbound') {
    ...
    await updateAccountAfterOutboundWhatsApp(
      supabase,
      context.account,
      message,
      excerpt
  ...
  if (message.direction === 'inbound') {
    ...
    await updateAccountAfterInboundWhatsApp(
      supabase,
      context.account,
      message,
      excerpt,
...
export async function processCrmWhatsAppSync(
  supabase: SupabaseClient,
  payload: CrmWhatsAppSyncPayload
): Promise<CrmWhatsAppSyncResult> {
  await loadWhatsAppConnection(supabase, payload.connectionId)
  const jobId = await createSyncJob(supabase, payload)

  const result: CrmWhatsAppSyncResult = {
    jobId,
    jobStatus: 'running',
  ...
  try {
    for (const message of payload.messages) {
      try {
        const processed = await processWhatsAppSyncMessage(supabase, message)

        if (!processed.matched) {
          result.unmatchedMessages += 1
          result.errors.push({
            providerMessageId: message.providerMessageId,
            reason: 'No CRM account matched the WhatsApp payload.',
          })
          continue
        }
        ...
        result.errors.push({
          ...
          reason:
            error instanceof Error
              ? error.message
              : 'Unexpected WhatsApp sync processing error.',
        })
      }
    }
    ...
    const failedResult: CrmWhatsAppSyncResult = {
      ...result,
      jobStatus: 'failed',
      ...
      errors: [
        {
          ...
          reason:
            error instanceof Error
              ? error.message
              : 'Unexpected CRM WhatsApp sync failure.',
        },
      ],
    }
