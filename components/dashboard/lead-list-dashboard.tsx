...
async function requestLeads(filters: LeadFetchFilters, signal: AbortSignal): Promise<Lead[]> {
  const response = await fetch(buildLeadsUrl(filters), { method: 'GET', cache: 'no-store', signal })
  const payload = (await response.json().catch(() => null)) as Lead[] | { error?: string; details?: string } | null
  if (!response.ok) {
    const message = payload && !Array.isArray(payload) ? payload.details ?? payload.error : null
...
function getScore(lead: Lead) {
  return getLeadAverageScore(lead)
}
    ...
    async function loadLeads() {
      ...
      try {
        ...
        if (!controller.signal.aborted) {
          ...
          if (isPristine) {
            setCatalogLeads((curr) => curr.length === 0 ? nextLeads : curr)
          }
          setSelectedLeadId((curr) =>
            nextLeads.some((l) => l.id === curr) ? curr : (nextLeads[0]?.id ?? null)
          )
        }
      } catch (error) {
        if (!controller.signal.aborted) {
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="grid w-full max-w-xl gap-3 sm:grid-cols-3">
            ...
            <div className="rounded-[1.35rem] border border-border bg-surface-strong p-4">
              <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted">Leads visíveis</p>
              <p className="mt-3 text-3xl font-semibold tracking-[-0.05em]">{visibleCount}</p>
            </div>
            <div className="rounded-[1.35rem] border border-border bg-surface-strong p-4">
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm leading-6 text-muted">Clique em um card para ver detalhes.</p>
            </div>
          ...
          {errorMessage && (
            <div className="rounded-[1.1rem] border border-warning/35 bg-warning/10 px-4 py-3 text-sm leading-6 text-warning">
              {errorMessage}
            </div>
          )}
                  ...
                  <div
                    ...
                    className={`relative cursor-pointer rounded-[1.35rem] border transition-all duration-200 ${
                      isSelected
                        ...
                        : isChecked
                          ? 'border-accent/50 shadow-[0_2px_8px_rgba(17,24,18,0.06),0_0_0_1px_rgba(17,24,18,0.04)]'
                          : 'border-border shadow-[0_2px_8px_rgba(17,24,18,0.06),0_0_0_1px_rgba(17,24,18,0.04)] hover:shadow-[0_8px_24px_rgba(17,24,18,0.10)] hover:-translate-y-0.5'
                    }`}
                    ...
                    <div className="px-5 pt-4 pb-3 pl-10">
                      ...
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="text-right shrink-0">
                          <span className={`text-4xl font-bold leading-none tracking-[-0.04em] ${scoreTone(lead)}`}>
                            {scoreStr}
                          </span>
                        </div>
                      </div>
                      ...
                      {/* Company name */}
                      <h3 className="mt-2 text-xl font-semibold tracking-[-0.03em] leading-tight">
                        {lead.name}
                      </h3>
                      ...
                      <div className="mt-3 flex flex-wrap items-center gap-3 text-xs text-muted">
                        ...
                        {lead.site && (
                          <span className="flex items-center gap-1 truncate max-w-[160px]">
                            <span>🌐</span> {lead.site.replace(/^https?:\/\//, '')}
                          </span>
                        )}
                      </div>
                      ...
                      <div className="mt-3 h-1 w-full rounded-full bg-border overflow-hidden">
                        <div
                          className={`h-full rounded-full ${scoreBarColor(lead)} transition-all duration-300`}
          ...
          >
            ...
            <div className="flex items-center justify-between border-b border-border px-6 py-4">
              ...
              <button
                type="button"
                onClick={() => setSlideOpen(false)}
                className="ml-4 shrink-0 rounded-full border border-border bg-surface-strong p-2 text-muted transition hover:border-accent/40 hover:text-foreground"
