...
**Mode:** Ecosystem — standard 2025/2026 stack for Python scraper + Next.js dashboard + Supabase
...
**Supabase client pattern — critical detail:**
...
**SSR vs client queries for this project:**

This is a single-user internal tool with no public pages. The recommended pattern is: fetch lead data in Server Components (avoids client-side waterfalls), use Client Components only for interactive parts (filters, status updates, pitch generation trigger). Route Handlers act as the API layer for Python scraper writes.
...
**Playwright async vs sync:**

Use **async Playwright** (`async_playwright`) even though the scraper is a CLI tool. Reason: site analysis involves multiple concurrent operations per lead (screenshot, DOM inspection, WhatsApp button check). Async enables running these in parallel per lead, significantly reducing total runtime.

Pattern: `asyncio.run(main())` as entry point, `async with async_playwright() as p:` for browser lifecycle. Browser context per-batch, not per-page — reuse contexts across leads in the same run.

**Anti-bot config for site analysis (not aggressive scraping):**

The target sites are small local business websites — most have minimal or no bot detection. Required mitigations:
1. `tf-playwright-stealth` to patch fingerprints
2. Realistic `user_agent` string (Chrome on Windows, not HeadlessChrome)
3. `viewport` set to `{'width': 1280, 'height': 800}`
4. `locale='pt-BR'` and `timezone_id='America/Sao_Paulo'` in browser context
5. Random 1–3 second delays between domain requests

No need for residential proxies or CAPTCHA solvers for this use case.
...
Use **non-streaming** (`client.messages.create()`). Reasons:
1. Pitches are max 300 tokens — response arrives in under 2 seconds even without streaming.
2. Streaming adds complexity (SSE handling in API route, ReadableStream in client) with no UX benefit at this output length.
3. Non-streaming is simpler to implement, test, and store (you get the full string, save to DB, return to client in one operation).

Only consider streaming if pitches grow to 1,000+ tokens, which is outside this project's scope.

**Where to call the API:**

Trigger pitch generation from a **Next.js Route Handler** (`app/api/generate-pitch/route.ts`), not directly from the browser. This keeps the `ANTHROPIC_API_KEY` server-side only. The route handler calls the Anthropic API, writes the pitch to Supabase, and returns it to the dashboard.
...
python>=3.11,<3.13
playwright>=1.49
tf-playwright-stealth==2.0.2       # pin — v2 API changed from v1
supabase==2.28.3                   # or latest 2.x; avoid 3.0.0a1 (pre-release)
requests>=2.31
tqdm>=4.66
python-dotenv>=1.0
anthropic>=0.96.0
